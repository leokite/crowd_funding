...
contract('CrowdFunding', ([owner, investor1, investor2]) => {
  const DURATION = 1800; // 30 munites
  ...
  it('should fail when checkGoalReaced is called before campaign end', async() => {
    instance.checkGoalReached({ from: owner }).should.be.rejectedWith('revert');
  });

  it('should fail when checkGoalReaced is called by other than owner', async() => {
  ...
  it('should fail when fund is called after the campaign end', async () => {
